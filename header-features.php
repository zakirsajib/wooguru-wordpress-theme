...
<body <?php body_class('fade-in')?>>
    <div class="feature-masthead <?php the_field('color_scheme')?>">
        <header class="header <?php the_field('menu_color_scheme')?> clearfix">
            <div class="row">
                <div class="small-12 column">
                    <?php $menu_color = get_field('menu_color_scheme')?>
                    <?php if($menu_color =='light'):?>
                    <a href="<?php echo home_url();?>" title="To the homepage" class="left"><img src="<?php echo get_template_directory_uri()?>/images/logo-dark.svg" width="90" height="19" alt="WooGuru.net WooCommerce Support"></a>
                    <?php else:?>
                ...
                <?php if ( have_posts() ) : ?>
					<?php while(have_posts()) : the_post();?>   
						...
						<?php if(get_field('header_title')):?>
							<h1 class="text-center push-40"><?php the_field('header_title')?></h1>
						<?php else:?>
							<h1 class="text-center push-40"><?php the_title()?></h1>
						<?php endif;?>
						<h2 class="h1 text-center subheader"><?php the_field('header_sub_title')?></h2></div>
						<?php endwhile;?>
				<?php else : ?>
